...
public class LadderRecipe extends CustomRecipe {
    ...
    public boolean matches(CraftingInput craftingInput, Level level) {
        ...
        boolean patternMatches = isStick(pattern[0][0]) && pattern[0][1].isEmpty() && isStick(pattern[0][2])
                              && isStick(pattern[1][0]) && isPlanks(pattern[1][1]) && isStick(pattern[1][2])
                              && isStick(pattern[2][0]) && pattern[2][1].isEmpty() && isStick(pattern[2][2]);
